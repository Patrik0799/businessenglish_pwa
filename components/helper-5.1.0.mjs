 ...
 * version 5.0.0 (06.04.2020):
 * - removed helper function 'loading' (moved back to ccm framework)
 * - bug fix for helper function 'hasPermission' when no user instance is passed
 ...
 * version 4.2.0 (28.03.2020):
 * - added helper function 'hasPermission'
 * - added helper function 'isCreator'
 * version 4.1.1 (23.03.2020):
 * - bug fix for checkboxes in helper function 'fillForm'
 * version 4.1.0 (18.03.2020):
 * - added helper function 'progressBar'
 ...
 * - changed structure of returned result
 * - not backwards compatible with older forms of app URLs and embed codes
 * (for older version changes see helper-3.0.0.mjs)
 ...
 */
export async function action( action, context, ccm ) {
  ccm = framework( arguments );
 ...
 */
export function hasDomContact( instance, ccm ) {
  ccm = framework( arguments );
  return document.contains( ccm.context.root( instance ).root ) && ( hasParentContact( instance ) || !instance.parent )
}
 ...
 * @memberOf ModuleHelper.DataHandling
 */
export function cleanObject( obj, deep, ccm ) {
  ccm = framework( arguments );
 ...
 * @memberOf ModuleHelper.DataHandling
 */
export function decodeJSON( str, ccm ) {
  ccm = framework( arguments );
 ...
 * @memberOf ModuleHelper.DataHandling
 */
export function encodeJSON( json, ccm ) {
  ccm = framework( arguments );
 ...
 * @memberOf ModuleHelper.DataHandling
 */
export function filterData( data, mask, ccm ) {
  ccm = framework( arguments );
 ...
 * console.log( dataset );  // => { key: dataset_key, ... }
 * @example
 * // load of a user-specific dataset
 ...
 */
export async function dataset( settings={}, ccm ) {
  ccm = framework( arguments );
 ...
 * @memberOf ModuleHelper.DataWorkflow
 */
export async function onFinish( settings, results, ccm ) {
  ccm = framework( arguments );
  let instance, user;
  ...
  if ( settings.store && results ) {
    ...
    await ccm["25.5.2"].set( settings.store.settings, dataset );                         // store result data in datastore
 ...
 */
export function append( element, content, ccm ) {
  ccm = framework( arguments );
 ...
 * @memberOf ModuleHelper.DomManipulation
 */
export function prepend( element, content, ccm ) {
  ccm = framework( arguments );
 ...
 * @memberOf ModuleHelper.DomManipulation
 */
export function replace( element, other, ccm ) {
  ccm = framework( arguments );
 ...
 */
export function fillForm( element, data, ccm ) {
  ccm = framework( arguments );
 ...
 * @memberOf ModuleHelper.InputElements
 */
export function formData( element, settings = {}, ccm ) {
  ccm = framework( arguments );
 ...
 */
export function appURL( component, config = {}, website = 'https://ccmjs.github.io/digital-maker-space/app.html', ccm ) {
  ccm = framework( arguments );
 ...
 * @memberOf ModuleHelper.HandoverApp
 */
export function decomposeEmbedCode( embed_code, ccm ) {
  ccm = framework( arguments );
 ...
 * @memberOf ModuleHelper.HandoverApp
*/
export function embedCode( component, config = {}, noscript, ccm ) {
  ccm = framework( arguments );
 ...
 * @memberOf ModuleHelper.Security
 */
export function protect( html, ccm ) {
  ccm = framework( arguments );
